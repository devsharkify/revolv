...
function corsHeaders(origin) {
  // In production, restrict to your actual domain: 'https://revolv.com'
  // For pilot/testing, allow all origins:
...
export default async function handler(request) {
  ...
  try {
    ...
    // Safety: cap max_tokens so a rogue request can't drain your budget
    if (body.max_tokens && body.max_tokens > 4000) {
      body.max_tokens = 4000;
    }

    // Safety: only allow messages endpoint models we've approved
    const allowedModels = [
    ...
    if (body.model && !allowedModels.includes(body.model)) {
      body.model = 'claude-sonnet-4-20250514'; // fallback
    }
